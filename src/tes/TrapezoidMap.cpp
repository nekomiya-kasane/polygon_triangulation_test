...
struct Frame
{
  ...
  Frame() {}
  Frame(TrapezoidMap::RegionId regionId_, int depth_, int winding_)
      : regionId(regionId_), depth(depth_), winding(winding_)
  {}
};

float DetermineWindingOrder(const Vec2 *polyPoints, unsigned polyPointCount)
{
  float result = float(0.0);
  for (unsigned i = polyPointCount - 1, j = 0; j < polyPointCount; i = j, ++j)
  {
...
TrapezoidMap::TrapezoidMap(const std::vector<Vec2> &vertices,
      ...
      mIsValid(true)
{
  ...
  for (int i = 0; i < mRegions.GetCapacity(); ++i)
  {
  ...
  for (size_t i = 0; i < contours.size(); ++i)
  {
    ...
    if (size > 0)
    {
      char winding = DetermineWindingOrder(&mVertices[offset], size) >= 0.f ? 1 : -1;
  ...
  // while (!order.empty())
  //{
  //   int index = mRandom.Uint32() % order.size();
  //   std::swap(order[index], order.Back());

  //  random.push_back(order.Back());
  //  order.PopBack();
  //}

  std::vector<NodeId> rootId(vertices.size());
  std::vector<char> alreadyAdded(vertices.size());
  ...
  int phaseCount          = ComputeLogStarN(vertices.size());
  ...
  std::vector<bool> cache(mRegions.GetCapacity());
  ...
  while (!next.empty())
  {
    ...
    while (!current.empty())
    {
      ...
      int regionId = frame.regionId;
      int depth    = frame.depth;
      int winding  = frame.winding;
      ...
      if (regionId != -1 && cache[regionId] == false)
      {
        ...
        current.push_back(Frame(region->TopNeighbor[0], depth, winding));
        current.push_back(Frame(region->TopNeighbor[1], depth, winding));
        current.push_back(Frame(region->BotNeighbor[0], depth, winding));
        current.push_back(Frame(region->BotNeighbor[1], depth, winding));
        ...
        if (index < 2 && belowCache->Index[index + 1] != -1)
        {
          ...
          if (neighbor->Depth == -1)
          {
            next.push_back(Frame(neighborId, depth + 1, winding + mWindingOrder[region->RightEdge]));
          }
        }
        ...
        if (index > 0)
        {
          ...
          if (neighbor->Depth == -1)
          {
            next.push_back(Frame(neighborId, depth + 1, winding + mWindingOrder[region->LeftEdge]));
          }
        }
      }
    }
...
bool TrapezoidMap::IsAboveInternal(TrapezoidMap::VertexId idA, TrapezoidMap::VertexId idB) const
{
  ...
  if (a.y == b.y)
  {
    if (a.x == b.x)
    {
      ...
      int end  = mEndIndex[mFarthestEnd[idA]];
...
bool TrapezoidMap::IsLeft(EdgeId start, EdgeId end, const Vec2 &point) const
{
  ...
  return ((p2 - p1) ^ (point - p1)) > 0.f;
}

bool TrapezoidMap::IsLeftInternal(EdgeId start, EdgeId end, VertexId pointId) const
{
  VertexId bot = start;
  ...
  // determinant will be plasma in which case we need to test the top point
  // of the query point with respect to the original edge
  float det = ((p2 - p1) ^ (point - p1));
                               ...
                               RegionId rightParent)
{
  ...
  assert(leftParent == -1 && rightParent == -1);
  ...
  {
    assert(!"Error building trapezoid graph.");
    return false;
  }
...
TrapezoidMap::NodeId TrapezoidMap::MergeAbove(RegionId regionId, RegionId *parent)
{
  ...
  assert(above->BotNeighbor[1] == -1);
  assert(above->BotNeighbor[0] == regionId);
