...
public:
  ...
  // Internal chirality: clockwise for the boundary and CCW for the holes.
...
protected:
  ...
  double EvalSignedArea(double *points, uint32_t size);                                 /* positive if CCW */
  double EvalSignedArea(const Node *const A, const Node *const B, const Node *const C); /* positive if CCW */
  bool LocallyInside(const Node *const base, const Node *const point);
